...
async function tick() {
  const js = [
    ...
    '    if (nb) return JSON.stringify(Object.assign({state:"next"},mid(nb.getBoundingClientRect())));',
    '    return JSON.stringify({state:"ended-no-next"});',
    '  }',
    '  if (media.paused) {',
...
async function main() {
  ...
  while (true) {
    try {
      ...
      if (target.state === 'playing' || target.state === 'no-media' || target.state === 'ended-no-next' || target.state === 'no-btn') {
