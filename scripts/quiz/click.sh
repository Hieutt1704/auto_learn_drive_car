...
# quy đổi CSS px -> điểm màn hình (hệ số k phụ thuộc zoom trang Chrome hiện tại, toolbar offset=86pt — xem README.md cùng thư mục),
...
set px to round (winX + (cx * 1.0))
set py to round (winY + 86 + (cy * 1.0))
